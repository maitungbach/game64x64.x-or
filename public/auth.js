...
function hydrateStaticText() {
  ...
  if (subtitle) {
    subtitle.textContent = "Đăng nhập hoặc tạo tài khoản để lưu tên người chơi trên trình duyệt này.";
  }
  ...
  if (note) {
    note.textContent = "Đăng nhập được xử lý bởi máy chủ. Tài khoản thường giới hạn 1 phiên, riêng 5 tài khoản kiểm thử có thể đăng nhập song song.";
  }
...
async function handleRegister(event) {
  ...
  if (!result.ok) {
    ...
    if (result.status === 429) {
      setMessage(`Bạn thao tác quá nhanh. Thử lại sau ${formatRetryAfter(result.retryAfterSec)}.`, "error");
      return;
    }
    setMessage("Không tạo được tài khoản.", "error");
    return;
